...
export function RuleDetailView({ ruleId }: { ruleId: string }) {
  ...
  const handleRemoveFromElastic = async () => {
    ...
    try {
      const res = await fetch(`/api/rules/${ruleId}/delete-elastic`, { method: "POST" });
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDuplicate = async () => {
    setDuplicating(true);
    try {
      const res = await fetch(`/api/rules/${ruleId}/duplicate`, { method: "POST" });
      const data = await res.json();
      if (res.ok) {
        addToast("success", "Rule duplicated");
        openRule(data.rule.id, data.rule.title);
      } else {
  ...
  const checkSync = async () => {
    ...
    try {
      const res = await fetch(`/api/rules/${ruleId}/elastic-check`);
      const data = await res.json();
  ...
  const handlePullElastic = async (force = false) => {
    setPulling(true);
    try {
      const res = await fetch(`/api/rules/${ruleId}/pull-elastic`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
