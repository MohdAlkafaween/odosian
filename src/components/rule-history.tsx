...
// The record list + expandable detail (including the enhance query
// comparison) shared by the rule's own page and its standalone
// /rules/[id]/history page — built once here so the two don't drift.
export function RuleHistory({ ruleId, ruleLanguage, ruleTitle }: { ruleId: string; ruleLanguage: string; ruleTitle?: string }) {
  ...
  useEffect(() => {
    if (!ruleId) return;
    fetch(`/api/analysis?ruleId=${ruleId}&limit=50&sortDir=desc`)
      .then((r) => r.json())
      .then((d) => {
  ...
  if (analyses.length === 0) {
    return <p className="text-text-muted">No AI activity on this rule yet.</p>;
  }
            ...
            {expanded && (
              <CardBody className="pt-0 space-y-4">
                ...
                <div className="flex items-center gap-4 pt-2 border-t border-border text-[10px] text-text-muted">
                  ...
                  <button
                    onClick={() => openAnalysisTab(a.id, a.analysisType, ruleId, ruleTitle)}
